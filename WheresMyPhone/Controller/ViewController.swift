...
//        trackSliderChangesObservable()
    ...
    @IBAction func scanButton(_ sender: Any) {
        ...
        observable
            .subscribeOn(MainScheduler.instance)
            .subscribe(onNext: { (device) in
            self.viewModel.addDevice(device)
                DispatchQueue.main.async {
                    self.tableView.reloadData()
                }
        }, onError: { (error) in
            AlertManager.errorAlert(withError: WMPError.deviceNotFound, self)
        }, onCompleted: {
            print("Completed scan publish")
        }) {
            print("Disposed scan publish")
        }.disposed(by: disposeBag)
    }
...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.movesLabel.text = "\(vm.coordinates.value.count)"
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectedDevice.isSelected.accept(true)
        trackCoordinatePublishSubject.onNext(selectedDevice.coordinates.value.last!)
        
        let connectedDevice = deviceManager.connectTo(viewModel.devices.value[indexPath.row])
        connectedDevice.asObservable()
            .subscribe(onNext: { (device) in
                print("Got device: \(device), isSelected: \(device.isSelected.value)")
            }, onError: { (error) in
                AlertManager.errorAlert(withError: WMPError.deviceNotFound, self)
            }, onCompleted: {
                print("Completed")
